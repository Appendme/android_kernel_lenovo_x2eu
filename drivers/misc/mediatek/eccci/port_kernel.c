	...
	case IPC_RPC_GET_RF_CLK_BUF_OP:
		{
			...
			if (count != CLKBUF_MAX_COUNT) {
				...
				if (node) {
					of_property_read_u32(node, "buffer1", (u32 *) &vals[0]);
					of_property_read_u32(node, "buffer2", (u32 *) &vals[1]);
					of_property_read_u32(node, "buffer3", (u32 *) &vals[2]);
					of_property_read_u32(node, "buffer4", (u32 *) &vals[3]);
				} else {
