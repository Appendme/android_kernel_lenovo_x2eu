...
static void process_dbg_opt(const char *opt)
{
	...
	if (0 == strncmp(opt, "regr:", 5)) {
		...
		if (enable == 1) {
			...
			pr_info("dump path status:");
			for (i = 0; i < module_num; i++)
				pr_info("%s-", ddp_get_module_name(modules[i]));
			...
			if (dbg_log_level < 3)
				...
				dbg_log_level = 0;
			pr_info("DDP: dbg_log_level=%d\n", dbg_log_level);
			sprintf(buf, "dbg_log_level: %d\n", dbg_log_level);
		} else if (enable == 8) {
			;
		} else if (enable == 9) {
			gOVLBackground = 0xFF0000FF;
			pr_info("DDP: gOVLBackground=%d\n", gOVLBackground);
			...
			pr_info("old status=%d, ovl1 status=%d\n", ovl_status, ovl_get_status());
...
{
	const int debug_bufmax = 16 * 1024 * 3 * 2 * 2;
	static int n;
	static char *debug_buffer;

	if (*ppos == 0) {
		debug_buffer = vmalloc(debug_bufmax);
		...
		dump_to_buffer = 0;
		n += scnprintf(debug_buffer + n, debug_bufmax - n, "%s\n", dprec_logger_get_dump_addr());
	...
	if (*ppos >= n) {
		...
		ASSERT(debug_buffer != NULL);
		return simple_read_from_buffer(buf, size, ppos, debug_buffer, n);
	}
}
