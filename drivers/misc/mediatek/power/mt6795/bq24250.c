...
kal_uint32 bq24250_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
	...
	battery_log(BAT_LOG_FULL, "[bq24250_read_interface] Reg[%x]=0x%x\n", RegNum, bq24250_reg);

    bq24250_reg &= (MASK << SHIFT);
    *val = (bq24250_reg >> SHIFT);

	battery_log(BAT_LOG_FULL, "[bq24250_read_interface] val=0x%x\n", *val);
...
kal_uint32 bq24250_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = bq24250_read_byte(RegNum, &bq24250_reg);
	battery_log(BAT_LOG_FULL, "[bq24250_config_interface] Reg[%x]=0x%x\n", RegNum-bq24250_CON0, bq24250_reg);
	...
	battery_log(BAT_LOG_FULL, "[bq24250_config_interface] write Reg[%x]=0x%x\n", RegNum-bq24250_CON0, bq24250_reg);

    // Check
    //bq24250_read_byte(RegNum, &bq24250_reg);
    /*battery_log(BAT_LOG_CRTI, "[bq24250_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24250_reg));*/
...
{
    int i=0;
	battery_log(BAT_LOG_CRTI, "[bq24250] ");

#if 1
    for (i=0;i<bq24250_REG_NUM;i++)
    {
        bq24250_read_byte(i+bq24250_CON0, &bq24250_reg[i]);
		battery_log(BAT_LOG_CRTI, "[0x%x]=0x%x ", i, bq24250_reg[i]);
    }
...
static int bq24250_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	battery_log(BAT_LOG_CRTI, "[bq24250_driver_probe]\n");
...
static ssize_t store_bq24250_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	battery_log(BAT_LOG_FULL, "[store_bq24250_access]\n");

    if(buf != NULL && size != 0)
    {
		battery_log(BAT_LOG_FULL, "[store_bq24250_access] buf is %s and size is %d\n", buf, size);
        reg_address = simple_strtoul(buf,&pvalue,16);

        if(size > 3)
        {
            reg_value = simple_strtoul((pvalue+1),NULL,16);
			battery_log(BAT_LOG_FULL, "[store_bq24250_access] write bq24250 reg 0x%x with value 0x%x !\n", reg_address, reg_value);
        ...
        else
        {
			...
			battery_log(BAT_LOG_FULL, "[store_bq24250_access] read bq24250 reg 0x%x with value 0x%x !\n", reg_address, g_reg_value_bq24250);
			battery_log(BAT_LOG_FULL, "[store_bq24250_access] Please use \"cat bq24250_access\" to get value\r\n");
        }
    }
    return size;
}
...
static int bq24250_user_space_probe(struct platform_device *dev)
{
	...
	battery_log(BAT_LOG_CRTI, "******** bq24250_user_space_probe!! ********\n");
...
static int __init bq24250_init(void)
{
	...
	battery_log(BAT_LOG_CRTI, "[bq24250_init] init start\n");

    i2c_register_board_info(2, &i2c_bq24250, 1);

    if(i2c_add_driver(&bq24250_driver)!=0)
    {
		battery_log(BAT_LOG_CRTI, "[bq24250_init] failed to register bq24250 i2c driver.\n");
    }
    else
    {
		battery_log(BAT_LOG_CRTI, "[bq24250_init] Success to register bq24250 i2c driver.\n");
    }

    // fan5405 user space access interface
    ret = platform_device_register(&bq24250_user_space_device);
    if (ret) {
		battery_log(BAT_LOG_CRTI, "****[bq24250_init] Unable to device register(%d)\n", ret);
        return ret;
    }
    ret = platform_driver_register(&bq24250_user_space_driver);
    if (ret) {
		battery_log(BAT_LOG_CRTI, "****[bq24250_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
