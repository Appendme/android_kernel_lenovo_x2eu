...
int bq27531_write_byte(kal_uint8 cmd, kal_uint8 writeData)
{
   ...
   /*battery_log(BAT_LOG_CRTI, "[bq27531] bq27531_write_byte,cmd=0x%x, data=0x%x\n", cmd, writeData));*/
	...
	/*battery_log(BAT_LOG_CRTI, "[bq27531] bq27531_write_byte,ret=%d\n", ret));*/
    if (ret < 0)
    {
...
int bq27531_read_bytes(kal_uint8 slave_addr, kal_uint8 *returnData, kal_uint32 len)
{
	...
	battery_log(BAT_LOG_CRTI, "ww_debug read 0x%x :", returnData[0]);
	ret = i2c_master_send(new_client, &returnData[0], ((len-1)<<8 | 1));
	/*battery_log(BAT_LOG_CRTI, "[bq27531] bq27531_read_byte,ret=%d, data=0x%x\n", ret, cmd_buf[0]));*/
	if (ret < 0)
	{
		//new_client->addr = new_client->addr & I2C_MASK_FLAG;
		new_client->ext_flag=0;
		...
		battery_log(BAT_LOG_CRTI, "[bq27531] bq27531_read_byte,ret<0 err\n");
		mutex_unlock(&bq27531_i2c_access);
		//kfree(buf);
	...
	for(ret=0;ret<len-1;ret++)
		battery_log(BAT_LOG_CRTI, " 0x%x", returnData[ret]);
...
int bq27531_write_bytes(kal_uint8 slave_addr, kal_uint8* writeData, kal_uint32 len)
{
	...
	battery_log(BAT_LOG_CRTI, "ww_debug write:");
	for(ret=0;ret<len;ret++)
		battery_log(BAT_LOG_CRTI, " 0x%x", writeData[ret]);
	battery_log(BAT_LOG_CRTI, "\n");
...
int bq27531_write_single_bytes(kal_uint8 slave_addr, kal_uint8* writeData, kal_uint32 len)
{
	...
	battery_log(BAT_LOG_CRTI, "ww_debug single write:");
	for(i=0;i<len-1;i++)
	{
		buf[0] = writeData[0] + i;
		buf[1] = writeData[i+1];

		battery_log(BAT_LOG_CRTI, "data1=0x%x, data2=0x%x  |  ", buf[0], buf[1]);
...
int bq27531_read_ctrl(kal_uint16 cmd, kal_uint16 *returnData)
{
    ...
    if (ret < 0)
    {
        //new_client->addr = new_client->addr & I2C_MASK_FLAG;
        new_client->ext_flag=0;
		battery_log(BAT_LOG_CRTI, "CL bq27531_get_ctrl_dfver err1 cmd=%x\n", cmd);
        mutex_unlock(&bq27531_i2c_access);
        return 0;
    }
    ...
    if (ret < 0)
    {
        //new_client->addr = new_client->addr & I2C_MASK_FLAG;
        new_client->ext_flag=0;
		battery_log(BAT_LOG_CRTI, "CL bq27531_get_ctrl_dfver err2 cmd=%x\n", cmd);
        mutex_unlock(&bq27531_i2c_access);
        return 0;
    }
...
int bq27531_write_ctrl(kal_uint16 cmd, kal_uint16 writeData)
{
	...
	for(ret=0;ret<5;ret++)
		battery_log(BAT_LOG_CRTI, "ww_debug data[%d]=0x%x\n", ret, write_data[ret]);
...
kal_uint32 bq27531_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
	...
	battery_log(BAT_LOG_FULL, "[bq27531_read_interface] Reg[%x]=0x%x\n", RegNum, bq27531_reg);

    bq27531_reg &= (MASK << SHIFT);
    *val = (bq27531_reg >> SHIFT);

	battery_log(BAT_LOG_FULL, "[bq27531_read_interface] val=0x%x\n", *val);
...
kal_uint32 bq27531_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = bq27531_read_byte(RegNum, &bq27531_reg);
	battery_log(BAT_LOG_FULL, "[bq27531_config_interface] Reg[%x]=0x%x\n", RegNum, bq27531_reg);

    bq27531_reg &= ~(MASK << SHIFT);
    bq27531_reg |= (val << SHIFT);

    ret = bq27531_write_byte(RegNum, bq27531_reg);
	battery_log(BAT_LOG_FULL, "[bq27531_config_interface] write Reg[%x]=0x%x\n", RegNum, bq27531_reg);

    // Check
    //bq27531_read_byte(RegNum, &bq27531_reg);
    /*battery_log(BAT_LOG_CRTI, "[bq27531_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq27531_reg));*/
...
unsigned int bq27531_get_ctrl_dfver(void)
{
	...
	kal_uint32 val = 0;
	 ...
	 battery_log(BAT_LOG_CRTI, "CL bq27531_get_ctrl_dfver ret=%d\n", ret);
	if(ret<=0)
		val = 0xffff;
...
short bq27531_get_averagecurrent(void)
{
	...
	battery_log(BAT_LOG_CRTI, "[bq27531] ww_debug cur = %d\n", (int) ret);
...
short bq27531_get_charge_voltage(void)
{
	...
	battery_log(BAT_LOG_CRTI, "[bq27531] ww_debug chg vol = %d\n", (short) ret);
...
void bq27531_dump_register(void)
{
    ...
    for (i=1;i<bq27531_REG_NUM;i++)
    {
		...
		battery_log(BAT_LOG_CRTI, "[0x%x]=0x%x ", bq27531_cmd_addr[i], bq27531_reg[i]);
    }
	battery_log(BAT_LOG_CRTI, "\n");
#endif
	...
	battery_log(BAT_LOG_CRTI, "ww_debug id = 0x%x\n", id);

	id = bq27531_get_ctrl_fwver();
	battery_log(BAT_LOG_CRTI, "ww_debug fwver = 0x%x\n", id);
#endif
...
static int bq27531_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	battery_log(BAT_LOG_CRTI, "[bq27531_driver_probe]\n");
	...
	I2CDMABuf_va = (u8 *)dma_alloc_coherent(NULL, 4096, &I2CDMABuf_pa, GFP_KERNEL);
    	if(!I2CDMABuf_va)
	{
			battery_log(BAT_LOG_CRTI, "[BQ27531] dma_alloc_coherent error\n");
	}
...
static ssize_t show_bq27531_download_fw(struct device *dev,struct device_attribute *attr, char *buf)
{
		...
		battery_log(BAT_LOG_CRTI, "[show_bq27531_download_fw] chi id =0x%x, fw ver = 0x%x, dfver = 0x%x\n", chipid, fwver, dfver);
		...
		battery_log(BAT_LOG_CRTI, "[show_bq27531_download_fw] bq27531_fw_update_status =0x%x, tmp =0x%x\n", bq27531_fw_update_status, tmp);
}
...
static ssize_t store_bq27531_download_fw(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	battery_log(BAT_LOG_CRTI,  "[store_bq27531_download_fw] in\n");
    if(buf != NULL && size != 0)
    {
        download_cmd = simple_strtoul(buf,&pvalue,16);
		battery_log(BAT_LOG_CRTI, "[store_bq27531_download_fw] buf is %s and size is %d,cmd is %d\n", buf, size, download_cmd);

		do
		{
           bq27531_fw_update_status = bq27531_check_fw_ver();
		   i++;
		   battery_log(BAT_LOG_CRTI, " [store_bq27531_download_fw] download times is %d\n", i);
		}
...
static int bq27531_user_space_probe(struct platform_device *dev)
{
	...
	battery_log(BAT_LOG_CRTI, "******** bq27531_user_space_probe!! ********\n");
...
static int __init bq27531_init(void)
{
	...
	battery_log(BAT_LOG_CRTI, "[bq27531_init] init start\n");

    i2c_register_board_info(BQ27531_BUSNUM, &i2c_bq27531, 1);

    if(i2c_add_driver(&bq27531_driver)!=0)
    {
		battery_log(BAT_LOG_CRTI, "[bq27531_init] failed to register bq27531 i2c driver.\n");
    }
    else
    {
		battery_log(BAT_LOG_CRTI, "[bq27531_init] Success to register bq27531 i2c driver.\n");
    }

    // fan5405 user space access interface
    ret = platform_device_register(&bq27531_user_space_device);
    if (ret) {
		battery_log(BAT_LOG_CRTI, "****[bq27531_init] Unable to device register(%d)\n", ret);
        return ret;
    }
    ret = platform_driver_register(&bq27531_user_space_driver);
    if (ret) {
		battery_log(BAT_LOG_CRTI, "****[bq27531_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
