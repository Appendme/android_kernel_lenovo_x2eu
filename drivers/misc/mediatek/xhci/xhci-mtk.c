...
int mtk_get_iddig_in_staus(void)
{
	return mt_get_gpio_in(iddig_gpio);
}
...
static bool mtk_is_over_current(void)
{
	...
	if(vol < 4200){
		mtk_xhci_mtk_log("over current occurs, voltage(%d) vbus_on(%d)\n", vol, vbus_on);
		return true;
	}
...
static int xhci_driver_load = 0;
void cc_decoder_mode_on(void)
{
	int ret = 0;
    /*mtk_idpin_cur_stat = (mtk_is_charger_4_vol())? IDPIN_IN_DEVICE : IDPIN_IN_HOST;

    printk("justin test for mtk_idpin_cur_stat(%d)\n",mtk_idpin_cur_stat);
    ...
    if(mtk_idpin_cur_stat==IDPIN_IN_DEVICE)
    {
        printk("there must some wrong with device detect,force to change mtk_idpin_cur_stat\n");
        mtk_idpin_cur_stat = IDPIN_IN_HOST;
    }
    
	ret = mtk_xhci_driver_load();
	if (!ret) {
        printk("justin test for xhci driver load sucessful\n");
...
void cc_decoder_mode_off(void)
{

	printk("%s:xhci_driver_load = %d, vbus_on = %d\n", __func__, xhci_driver_load, vbus_on);
	if(xhci_driver_load == 0 || vbus_on == 0)
		return;

	mtk_xhci_disPortPower();
    /*do{
        msleep(500);
    }while(mtk_is_hub_active());*///temp remove for otg sometimes unnormal.
	       ...
	       if(has_charge)
			...
			if(retry==retry_time)
			{
				if(pool_over_4v >=retry_time){  //stable device 
					...
					//mtk_xhci_mtk_log("charge device detected !!!\n");
					break;
				}else if(pool_less_4v>=retry_time){   //stable host
					mtk_idpin_cur_stat = IDPIN_IN_HOST;  
					//mtk_xhci_mtk_log("host device detected !!!\n");
					break;
